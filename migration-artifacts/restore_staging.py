#!/usr/bin/env python3
"""Restore Luxury Strand Haven dump into luxurystrand_staging on fleet-postgres."""
import json
import os
import subprocess
import sys
...
DUMP = Path(os.environ.get("LS_DUMP_DIR", "/data/coolify/luxurystrand-staging/dumps"))
DB = os.environ.get("LS_DB_NAME", "luxurystrand_staging")
ROLE = os.environ.get("LS_DB_ROLE", DB)
SUPABASE_HOST = os.environ.get("LS_SUPABASE_HOST", "https://rwpckvkfvdgtmfitavao.supabase.co")
...
def rewrite_storage_urls():
    ...
    host = SUPABASE_HOST.rstrip("/")
    for table in ("site_settings", "store_settings"):
        try:
            psql(
                f"""
                UPDATE public.{table}
                SET value = replace(value::text, '{host}', '')::jsonb
