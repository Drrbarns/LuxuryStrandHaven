// Luxury Strand Haven - network-first pages; no HTML shell cache (playbook section 16)
const CACHE_VERSION = 'sw-v2.5-luxury';
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    body: data.body || 'New update from Luxury Strand Haven',
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'Luxury Strand Haven', options)
  );
});
